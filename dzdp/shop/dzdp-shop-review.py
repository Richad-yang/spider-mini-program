...
class DzdpReviewFontDecrypt(object):
    """
    大众点评店铺详情页,css字体加密解密方式
    """

    def __init__(self, source_url, source_text):
        # 商家评论详情页 url
        self.url = source_url
        # 商家评论详情页源码
        self.html = source_text
        # 页面字体大小
        self.font_size = 14
        # 页面引用的 css 文件
        self.css = str()
    ...
    def __get_svg_html(self):
        ...
        if result:
            ...
            # 正则匹配商家地址使用的 svg 文件 url  bb[class^="wz"]
            ...
            # 正则匹配商家电话号码使用的 svg 文件 url cc[class^="os"]
            ...
            # 正则匹配评论使用的 svg 文件 url  svgmtsi[class^="ec"]
    ...
    def __get_font_map(self):
        ...
        """
        匹配评论 svg 文件中格式为 <text x="0" y="43">xxx</text> 下面两种方式可能交替进行
        """
        #review_svg_id_y_list = re.findall('<text x="0" y="(\d+)">(.*?)</text>', self.review_svg, re.S)
        #self.review_font_map = self.__review_class_to_font(review_class_list, review_svg_id_y_list)
        #print(self.review_svg)
        review_svg_y_words_list = re.findall('<path id="(\d+)" d="M0 (\d+) H600"/>', self.review_svg, re.S)
        review_svg_id_fonts_dc = dict(
            re.findall('<textPath xlink:href="#(\d+)" textLength=".*?">(.*?)</textPath>', self.review_svg, re.S)
        )
        self.review_font_map = self.__review_class_to_font(review_class_list, review_svg_y_words_list,
                                                           review_svg_id_fonts_dc)

        """
        匹配地址 svg 文件中格式为 <path id="(\d+)" d="M0 (\d+) H600"/> 的字段
        其中 id 的值对应 xlink:href="#(\d+)" 的值
        d="M0 (\d+) H600" 的值对应 background中 y轴的偏移量

        匹配地址 svg 文件中格式为 <textPath xlink:href="#(\d+)" textLength=".*?">(.*?)</textPath> 的字段
        (\d+) 对应为 css id 选择器，对应上面 <path> 中的 id
        (.*?) 对应一串中文字符串，
        还原后的字符 = 中文字符串[css 样式上中 x 的绝对值 / 字体大小]
        """
        # address_svg_y_words_list = re.findall('<path id="(\d+)" d="M0 (\d+) H600"/>', self.address_svg, re.S)
        # address_svg_id_fonts_dc = dict(
        #     re.findall('<textPath xlink:href="#(\d+)" textLength=".*?">(.*?)</textPath>', self.address_svg, re.S)
        # )
        # self.address_font_map = self.address_class_to_font(address_class_list, address_svg_y_words_list,
        #                                                    address_svg_id_fonts_dc)
        # TODO 样式经常会发生变化
        address_svg_y_words_list = re.findall('<text x="0" y="(\d+)">(.*?)</text>', self.address_svg, re.S)
        self.address_font_map = self.__address_class_to_font(address_class_list, address_svg_y_words_list)

        """
        匹配电话 svg 文件中格式为
        <text x="14 28 42 56 70 84 98 112 126 140 " y="33">0415678329</text>
        """
        ...
        self.tell_font_map = self.__tell_class_to_num(tell_class_list, tell_svg_x_words_list)

        print(f"地址map:{self.address_font_map}")
        print(f"评论map:{self.review_font_map}")
        print(f"电话map:{self.tell_font_map}")

    # def __address_class_to_font(self, class_list, y_words_list, words_dc):
    #     tmp_dc = dict()
    #     for class_name, class_x, class_y in class_list:
    #         for class_id, y in y_words_list:
    ...
    def __address_class_to_font(self, class_list, y_words_list):
        tmp_dc = dict()
        for class_name, class_x, class_y in class_list:
    ...
    # def __review_class_to_font(self, class_list, id_y_list):
    #     tmp_dc = dict()
    #     for class_name, class_x, class_y in class_list:
    #         for text_y, words in id_y_list:
    #             if int(text_y) >= abs(int(float(class_y))):
    #                 index = abs(int(float(class_x))) // self.font_size
    #                 tmp_dc[class_name] = words[index]
    #                 break
    ...
    def __review_class_to_font(self, class_list, id_y_list, words_dc):
        tmp_dc = dict()
        for class_name, class_x, class_y in class_list:
    ...
    def __tell_class_to_num(self, class_list, x_word_list):
        tmp_dc = dict()
        for class_name, class_x, class_y in class_list:
    ...
    def __get_shop_info(self):
        ...
        for i in tree.xpath('//div[@class="main-review"]'):
            ...
            write.write(f'【用户】：{user_name}')
            write.write('\r\n')
            write.write(f'【评论时间】：{review_time}')
            write.write('\r\n')
            print(f'星评：{star}')
            print(f'多维分数：{evaluation_list}')
            print(f'人均：{consumption_per_person}')
            print(f'评论：{review}')
            write.write(f'【评论】：{review}')
            write.write('\r\n')
...
if __name__ == '__main__':
    #url = 'http://www.dianping.com/shop/G2Ov8PAmclQ2Q3p5/review_all'.encode("utf-8").decode("latin1")
    url = 'http://www.dianping.com/shop/G2Ov8PAmclQ2Q3p5/review_all/p34'.encode("utf-8").decode("latin1")
    user_cookie = 'your cookie'.encode("utf-8").decode("latin1")
    headers = {
        ...
        'Referer': url.replace('/review_all', ''),
    ...
    dz = DzdpReviewFontDecrypt(url, response)
    dz.decrypt()
